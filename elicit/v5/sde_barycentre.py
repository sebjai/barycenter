...
class sde_barycentre():
    ...
    def estimate_omega(self, eta, n_iter=10, n_print=100, batch_size=256, rule="L2"):
        
        self.t_train = self.t.view(1,-1,1).to(self.dev).repeat(batch_size,1,1)  
        
        print_at_list = list(np.unique(np.array(np.ceil(1.21**(np.arange(0,56))-1), int)))
    ...
    #
    def train(self, batch_size = 1024, n_iter=1_000, n_print=100, rule="L2", rng_state=None):
        ...
        self.estimate_omega(self.eta['values'][-1], n_iter=n_iter, n_print=n_print, batch_size=batch_size, rule=rule)
    ...
    def plot_sample_qpaths(self, eta, batch_size = 4_096, filename=None):
        """
