...
class sde_barycentre():
    ...
    def __init__(self, X0, sde_model, pi, f=[], g=[], T=1, Ndt = 501):
        
        self.dev = sde_model[0].dev
        
        self.f = f
        self.g = g
        self.X0 = X0
        self.sde_model = sde_model
        self.K = len(self.sde_model)
        self.mu = []
        for k in range(self.K):
            self.mu.append(self.sde_model[k].nu['net'])
        self.sigma = self.sde_model[0].sigma['net']
    ...
    def mu_bar(self, x):
        ...
        for k in range(len(self.pi)):
            result +=self.pi[k]*self.mu[k](x)
            
        return result
        
    def step(self, t, x, mu, dW, dt):
        
        s = self.sigma(x)
        U = torch.linalg.cholesky(s)
        
        xp = x + mu(t, x) * dt  + torch.matmul(dW , U)
    ...
    def simulate(self, batch_size = 256):
        ...
        for i, t in enumerate(self.t[:-1]):
            ...
            dW = self.sqrt_dt * torch.randn(batch_size,self.d).to(self.dev)
            
            for k in range(self.K):
                
                X[:,i+1,:,k] = self.step(t, X[:,i,:,k], self.mu[k], dW, self.dt)
            
            X[:,i+1,:,-1] = self.step(t, X[:,i,:,-1], self.mu_bar,  dW, self.dt)
    ...
    def simulate_Qbar(self, batch_size = 256, init_randomize=False):
        ...
        for i, t in enumerate(self.t[:-1]):
            ...
            dW[:,i,:] = self.sqrt_dt * torch.randn(batch_size,self.d).to(self.dev)
            
            sigma = self.sigma(X[:,i,:])
            mu_bar = self.mu_bar(X[:,i,:])
            
            for k in range(self.K):
                dmu = (self.mu[k](X[:,i,:]) - mu_bar)/sigma
    ...
    def simulate_Q(self, batch_size = 256):
        ...
        for i, t in enumerate(self.t[:-1]):
            ...
            X[:,i+1,:] = self.step(t*ones, X[:,i,:], self.theta, dW, self.dt)
        ...
        for i in tqdm(range(n_iter)):
            ...
            if len(self.f+self.g) > 0:
                ...
                for k in range(len(self.g)):
                    int_g += eta[k]*self.int_tT(self.g[k](t, X).squeeze() )
