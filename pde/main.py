...
for a in pi_all:
    ...
    print(r"P(X_T<1)={0:0.3f}, {1:0.3f}".format(np.mean(paths[-1][-1,:]<x_bar),
                                                np.std(paths[-1][-1,:]<x_bar)/np.sqrt(n_sims)))
    print("******\n\n")
    ...
    def PlotPaths(paths, title=""):
        ...
        plt.axhline(x_bar, linestyle='--',color='r')
    ...
    filename = 'combined_{0:2.0f}'.format(100*pi[0])
