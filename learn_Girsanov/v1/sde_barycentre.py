...
class sde_barycentre():
    ...
    def get_net(self, nIn, nOut, output):
        ...
        obj = {'net' : net(nIn=nIn, nOut=nOut, output=output) }
    ...
    def simulate_Qbar(self, batch_size = 256):
        ...
        dW = torch.zeros(batch_size, self.Ndt, self.d)
        ...
        for i, t in enumerate(self.t[:-1]):
            ...
            dW[:,i,:] = self.sqrt_dt * self.Z.sample((batch_size,))
            ...
            X[:,i+1,:] = self.step(t, X[:,i,:], self.mu_bar, self.sigma, dW[:,i,:], self.dt)
    ...
    def simulate_Q(self, batch_size = 256):
        ...
        theta = lambda t,x : self.theta(torch.cat((t,x),axis=-1))
    ...
    def cumsum(self, x):
        ...
        zeros = torch.zeros(cx.shape[:-1]).unsqueeze(-1)
    ...
    def learn_theta(self, n_iter=10, n_print=100, batch_size=256):
        ...
        for i in tqdm(range(n_iter)):
            ...
            self.F.append(torch.mean(dQ_dQbar*F).detach().numpy())
